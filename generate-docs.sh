...
module="SplitView"
github="twodayslate/SplitView"
project="splitview.xcodeproj"
scheme="SplitView"

# get version number from podspec
version="$(egrep "^\s*s.version\s*" SplitView.podspec | awk '{ gsub("\"", "", $3);  print $3 }')"
today="$(date '+%Y-%m-%d')"
...
jazzy \
    ...
    --copyright "[© 2019 SplitView](https://github.com/twodayslate/SplitView/blob/master/LICENSE). (Last updated: $today)" \
    --author "twodayslate" \
